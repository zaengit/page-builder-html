...
export function AttributesPanel({e}:{e:EditorContextValue}){
  ...
  const applyCore=(name:'id'|'class'|'title',value:string)=>{const el=selected(e),d=el?.ownerDocument;if(!el||!d)return;if(name==='id'&&idConflict(d,el,value)){setIdError('ID already exists');return}setIdError('');e.mutate(node=>{const v=value.trim();if(v)node.setAttribute(name,v);else node.removeAttribute(name)})};
  const updateRow=(index:number,value:string)=>{const row=rows[index];setRows(v=>v.map((x,i)=>i===index?{...x,value}:x));e.mutate(el=>setSafeAttribute(el,row.name,value))};
  const removeRow=(index:number)=>{const row=rows[index];e.mutate(el=>el.removeAttribute(row.name));setRows(v=>v.filter((_,i)=>i!==index))};
  ...
  return <div className="p-3">
    <div className="space-y-3">
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-zinc-500">ID</span><input aria-label="Element ID" value={id} onChange={x=>setId(x.target.value)} onBlur={()=>applyCore('id',id)} className={input}/>{idError&&<span className="mt-1 block text-[10px] text-red-400">{idError}</span>}</label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Class</span><input aria-label="Element class" value={classes} onChange={x=>setClasses(x.target.value)} onBlur={()=>applyCore('class',classes)} className={input}/></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Title</span><input aria-label="Element title" value={title} onChange={x=>setTitle(x.target.value)} onBlur={()=>applyCore('title',title)} className={input}/></label>
    </div>
    <div className="mt-5 border-t border-zinc-800 pt-4"><div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Custom attributes</div>
      <div className="space-y-2">{rows.map((row,i)=><div key={`${row.name}-${i}`} className="grid grid-cols-[88px_1fr_30px] gap-1"><input value={row.name} readOnly className={`${input} text-zinc-500`}/><input aria-label={`Attribute ${row.name}`} value={row.value} onChange={x=>setRows(v=>v.map((r,j)=>j===i?{...r,value:x.target.value}:r))} onBlur={()=>updateRow(i,rows[i].value)} className={input}/><button onClick={()=>removeRow(i)} title={`Remove ${row.name}`} className="grid place-items-center rounded-md border border-zinc-800 text-zinc-500 hover:bg-red-500/10 hover:text-red-400"><Trash2 size={13}/></button></div>)}</div>
      ...
      <p className="mt-2 text-[10px] leading-4 text-zinc-600">Internal data-vpb attributes, style, and event handlers are not editable here.</p>
    </div>
  </div>;
}
